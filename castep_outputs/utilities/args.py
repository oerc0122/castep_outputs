...
from .dumpers import SUPPORTED_FORMATS
from ..parsers import _CASTEP_OUTPUT_NAMES, _CASTEP_FILE_FORMATS
...
AP = argparse.ArgumentParser(
    ...
    castep_outputs can parse most human-readable castep outputs including: {', '.join(_CASTEP_FILE_FORMATS)}"""
)
...
for output_name in _CASTEP_OUTPUT_NAMES:
...
def parse_args(to_parse: Sequence[str] = ()) -> argparse.Namespace:
    ...
    parse_all = args.inc_all or not any(getattr(args, f"inc_{typ}") for typ in _CASTEP_OUTPUT_NAMES)

    # Set all flags
    if parse_all and not to_parse:
        for typ in _CASTEP_OUTPUT_NAMES:
    ...
    for seed in args.seedname:
        seed = Path(seed)
        if seed.is_file() and (ext := seed.suffix[1:]) in _CASTEP_OUTPUT_NAMES:
...
def args_to_dict(args: argparse.Namespace) -> Dict[str, List[str]]:
    ...
    out_dict = {typ: getattr(args, typ) for typ in _CASTEP_OUTPUT_NAMES}
    return out_dict
